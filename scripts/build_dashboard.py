...
.metrics{display:grid;grid-template-columns:repeat(6,1fr);gap:1px;background:var(--line);border:1px solid var(--line);margin:26px 0}.metric{background:var(--panel);padding:17px}.metric b{display:block;font:700 35px Georgia,serif;margin-top:14px}
...
metrics.innerHTML=[["Accepted papers",data.paper_count],["Visualized papers",data.visualized_paper_count],["Wiki pages",data.wiki.page_count],["Graph links",data.graph.edges.length],["Scout runs",data.runs.length],["Scout spend",`$${data.scout_spend_usd.toFixed(2)}`]].map(x=>`<div class="metric"><span class="eyebrow">${x[0]}</span><b>${x[1]}</b></div>`).join("");
...
const candidateSummaryText = data.candidate_count ? `Latest scout found ${data.candidate_count} candidates${data.candidate_generated_at ? ` at ${data.candidate_generated_at}` : ""}. Recorded scout cost: ${data.candidate_cost.token_count} tokens, $${data.candidate_cost.money_cost_usd.toFixed(2)} ${data.candidate_cost.currency}.` : "No candidates recorded yet. Run `make scout` to populate this section.";
